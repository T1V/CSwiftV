...
public class CSwiftV {
    ...
    public init(with string: String, separator: String = ",", headers: [String]? = nil) {
        var standardString = string.replacingOccurrences(of: "\r\n", with: "\n")
        standardString = standardString.replacingOccurrences(of: "\r", with: "\n")
        var parsedLines = CSwiftV.records(from: standardString).map { CSwiftV.cells(forRow: $0, separator: separator) }
